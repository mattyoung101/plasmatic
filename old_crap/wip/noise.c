...
// orig source
// https://gist.github.com/Yukinyaa/bfb4a8a45cbba84da31ead7e93076920
// ported to C by Matt Young 2025 UQCS Hackathon
...
static inline int Fade(int t) // returns 0~4095
{
    int t0 = f[t >> 8];
    int t1 = f[Min(255, (t >> 8) + 1)];
    return t0 + (((t & 255) * (t1 - t0)) >> 8); // interpolates between t and t1.
                                                //t0 + (((t % 256) * (t1 - t0)) / 256)
}

static int Noise3D(int x, int y, int z)
{

        int X = x >> 16 & 255; // xi
        int Y = y >> 16 & 255; // yi
        int Z = z >> 16 & 255; // zi
        const int N = 1 << 16; // 65536 = 2^16
        x &= N - 1; // xf
        y &= N - 1; // yf
        z &= N - 1; // zf
...
static float Noise3Df(float x, float y, float z)
{
        return Noise3D((int)(x * 65536), (int)(y * 65536), (int)(z * 65536)) / 65536.f; //65536 = 2 ^ 16
}
